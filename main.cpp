...
int main() {
  ...
  Stack<int> stack(5);                     //degradation Stack and gave Type and Size
    ...
    while (!stack.StackIsEmpty()) {          //We will do a loop check to see if the stack elements are empty or not
        cout << stack.StackTop(x) << " ";            // Print the last stored item add space between them
